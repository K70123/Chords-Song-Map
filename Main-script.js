...
function renderUI() {
  ...
  liveSongs.forEach(songName => {
    ...
    li.addEventListener('click', function() {
      localStorage.setItem('currentSongTitle', songName);
    ...
    li.addEventListener('contextmenu', function(e) {
      ...
      liveSongs = liveSongs.filter(s => s !== songName);
  ...
  liveTitle.addEventListener('dblclick', function(e) {
    e.preventDefault();
  ...
  liveSongs.forEach(songName => {
    ...
    if (songObj) {
        ...
        if (addPartBtn) { 
          ...
          newAddPartBtn.addEventListener('click', function() {
            if (songObj) {
              ...
              showAllLiveSongsAndSections();
              showSongMap(songObj, songMapList, changeSongMapBtn, addPartBtn, e);
            }
          });
        }
      };
      ...
      function attachSongMapPartListeners(songObj, songMapList) {
        ...
        const list = liveSongMapContainer.querySelector('.songMapList');
      ...
      topContainer.appendChild(changeSongMapBtn);
      changeSongMapBtn.onclick = function(e) {
        showSongMap(songObj, songMapList, changeSongMapBtn, addPartBtn, e);
      };

      const openSongMapList = localStorage.getItem('songMapList');
      if (openSongMapList === songObj.name){
      ...
      if (Array.isArray(window.keyList)) {
        ...
        window.keyList.forEach(key => {
          ...
          else if (key === 'Num' && localStorage.getItem('nashvilleMode') === 'true'){
          ...
          keyBtn.addEventListener('click', function() {
            if (songObj) {
              ...
              if (keyBtn.textContent === 'Num') {
                ...
                const current = localStorage.getItem('nashvilleMode') === 'true';
                localStorage.setItem('nashvilleMode', !current);
                showAllLiveSongsAndSections();
              }
              else {
  ...
  const currentSongTitle = localStorage.getItem('currentSongTitle');
  ...
  function transposeChord(chord, fromKey, toKey) {
    ...
    const nashvilleMode = localStorage.getItem('nashvilleMode') === 'true';
    ...
    chordSpans.forEach(chordSpan => {
      chordSpan.addEventListener('dblclick', function(e) {
        ...
        chordSpan.addEventListener('blur', function() {
          ...
          if (currentSongObj) {
            currentSongObj.chords[sectionIdx].chords[chordIdx] = chordSpan.textContent.trim();
            saveData();
            renderUI();
          }
          // If the chordSpan is empty, remove it
  ...
  if (localStorage.getItem('liveSection') === 'true') {
    showAllLiveSongsAndSections();
    localStorage.removeItem('liveSection');
  }
}
